...
class Unet1D_cond(nn.Module):
    ...
    def __init__(
        ...
        if self.embedding_type == "mlp":
            self.cond_emb = nn.Linear(1, embedding_dim)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor = None, c: torch.Tensor = None) -> torch.Tensor:
        """
        Args:
        ...
        if self.use_cond:
            if self.embedding_type == "mlp":
                ...
                c = self.cond_emb(c.unsqueeze(1))
            else:
                t = sinusoidal_embedding(t, self.embedding_dim, scale=self.time_scale)  # (b,) -> (b, cond_channels)
                c = sinusoidal_embedding(c, self.embedding_dim, scale=self.cond_scale)  # (b,) -> (b, cond_channels)

            emb = torch.cat([t, c], dim=-1)  # (b, cond_channels*2)
        else:
            emb = sinusoidal_embedding(t, self.embedding_dim, scale=self.time_scale)
        emb = self.emb_in(emb)  # (b, cond_channels*2) -> (b, cond_channels)

        if len(x.shape) == 3:
            x = x.unsqueeze(2)  # make an assumption that channel is 1
